...
/**
 * Common handler for authentication/password failures to prevent brute force.
 */
async function handleAuthFailure(ip, res, now, message = '用户名或密码错误', code = 401) {
...
router.post('/login', async (req, res) => {
    try {
        ...
        if (rows.length > 0) {
            ...
            if (!isMatch) {
                return await handleAuthFailure(ip, res, now);
            }
...
router.post('/register', async (req, res) => {
    ...
    // Check if currently blocked (reuse existing logic)
    const attempts = loginAttempts.get(ip) || { count: 0, lastAttempt: 0 };
    if (attempts.count >= 3 && now - attempts.lastAttempt < 30000) {
        return res.status(429).json({
            "status": "fail",
            "code": 429,
            "message": '操作过于频繁，请30秒后再试',
            "data": {},
            "error": {}
        });
    }
    ...
    try {
        ...
        if (rows.length > 0) {
            return await handleAuthFailure(ip, res, now, '用户名已存在', 400);
        }
